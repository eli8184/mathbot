...
function renderMessageContent(content) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('\\(') && part.endsWith('\\)')) {
      // Parentheses indicate inline math
...
function App() {
  ...
  const [messages, setMessages] = React.useState([
    { role: "assistant", content: "Hello! My name is Professor AI, how can I assist you today?" },
  ]);
  ...
  const clearHistory = async () => {
    ...
    try {
      await fetch("http://localhost:3001/clear-history", { method: "POST" });
    } catch (e) {
    ...
    setMessages([{ role: "assistant", content: "Hello! My name is Professor AI, how can I assist you today?" }]);
  };
  ...
  return (
    ...
    <Container>
      <Typography variant="h4" align="center" gutterBottom>Professor AI Chatbot</Typography>
      ...
      <Paper style = {{maxHeight: 400, overflow: "auto", padding: 16, marginBottom: 16}}>
        <List>
          ...
          {messages.map((message, index) => (
            <ListItem key = {index}>
              <ListItemText
                primary={message.role === "user" ? "You" : "Prof. AI"}
                secondary={<span>{renderMessageContent(message.content)}</span>}
              />
            </ListItem>
          ))}
        </List>
